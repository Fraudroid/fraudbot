...
class Device(object):
    ...
    def receive_sms(self, phone=DEFAULT_NUM, content=DEFAULT_CONTENT):
        ...
        :return:
        """
        if self.get_telnet() is not None:
            return
    ...
    # add by wangsonghe 2017.3.8
    def get_current_traffic(self, app):
        assert isinstance(app, App)
        self.logger.info("getting current device traffic...")
        current_traffic = self.get_adb().shell("ps | grep %s" % app.get_package_name())
        if current_traffic is None:
            return '0'
        self.logger.info('!!!!CURRENT TRAFFIC:  ' + current_traffic)
        if current_traffic is '':
            return None
        current_traffic = current_traffic.split(' ')
        current_traffic_info = list(set(current_traffic))
        current_traffic_info.sort(key=current_traffic.index)
        self.logger.info(current_traffic_info)
        current_pid = current_traffic_info[2]
        self.logger.info('current pid = ' + current_pid)
        status = self.get_adb().shell("cat /proc/%s/status" % current_pid)
        if status.find('Uid:\t'):
            uid = status[status.find('Uid:\t')+5:status.find('Uid:\t')+10]
        self.logger.info("current UID: " + uid)
        rcv_traffic = self.get_adb().shell("cat /proc/uid_stat/%s/tcp_rcv" % uid)
        self.logger.info("current recieve traffic: " + rcv_traffic)
        if 'No such file' in rcv_traffic:
            return '0'

        return rcv_traffic

    def get_current_state(self, app):
        ...
        try:
            ...
            current_traffic = self.get_current_traffic(app)
...
class DeviceState(object):
    ...
    def __init__(self, device, view_client_views, foreground_activity, background_services, current_traffic, tag=None, screenshot_path=None):
        self.device = device
